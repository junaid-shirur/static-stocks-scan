...
import gobackIcn from '../assets/goback_icn.svg'
type Details = {
    name: string
    tag: string
    color: string
...
const Detail: React.FC<Props> = ({ details, isVariables = false }) => {
    ...
    return (
        <>
            <div className="bg-gray-200 py-6 gap-x-5 flex flex-col items-center h-screen w-screen justify-center">
                <div className="md:min-w-[30vw] sm:min-w-[70vw] ">
                    <button onClick={() => navigate('/home')} className="inline-flex gap-2 items-center my-3">
                ...
                <div className="overflow-hidden bg-white shadow sm:rounded-md md:min-w-[30vw] px-4 py-4 sm:px-4 w-4ull">
                    ...
                    <label className={`bg-${details.color}-100 text-${details.color}-800 inline-flex rounded-full px-2 text-xs font-semibold leading-5`}>{details.tag}</label>
